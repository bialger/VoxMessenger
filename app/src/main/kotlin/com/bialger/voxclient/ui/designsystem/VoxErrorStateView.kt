...
import com.bialger.voxclient.databinding.ViewVoxErrorStateContentBinding

class VoxErrorStateView @JvmOverloads constructor(
    context: Context,
    attrs: AttributeSet? = null,
    defStyleAttr: Int = 0,
) : ConstraintLayout(context, attrs, defStyleAttr) {

    private val binding = ViewVoxErrorStateContentBinding.inflate(LayoutInflater.from(context), this)
    ...
    init {
        ...
        if (!subtitle.isNullOrBlank()) {
            setMessageText(subtitle)
        }
    }

    fun setTitleText(value: CharSequence) {
        binding.errorTitle.text = value
    }

    fun setMessageText(value: CharSequence) {
        binding.errorSubtitle.text = value
    }
}
